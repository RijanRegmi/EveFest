...
import type { IUser, ISupportMessage } from "@/types";
...
export default function AdminDashboardPage() {
  ...
  const [activeTab, setActiveTab] = useState<"users" | "events" | "support">("users");
  ...
  const [threadMessages, setThreadMessages] = useState<ISupportMessage[]>([]);
  ...
  const handleSupportReply = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
  ...
  return (
    <>
      ...
      <style jsx>{`
        ...
        .status-badge { display: inline-flex; align-items: center; gap: 0.35rem; font-size: 0.75rem; font-weight: 750; padding: 0.2rem 0.55rem; border-radius: var(--border-radius-sm); }
