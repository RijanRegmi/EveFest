...
router.get("/admin/threads", admin, async (_req, res, next) => {
    try {
        const threadUserIds = await SupportMessage.distinct("userId");
        const threads = await Promise.all(threadUserIds.map(async (userId) => {
        ...
        threads.sort((a, b) => new Date(b.latestMessage.createdAt).getTime() -
