...
export const getGroupMessages = async (req, res) => {
  try {
    ...
    // Verify user is a registered attendee or the host
    ...
    if (!booking && !isHost) {
      return res.status(403).json({ message: "Only registered attendees or the event host can view group chat." });
    }
...
export const sendGroupMessage = async (req, res) => {
  try {
    ...
    if (!booking && !isHost) {
      return res.status(403).json({ message: "Only registered attendees or the event host can send messages." });
    }
